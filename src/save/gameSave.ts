...
export const normalizeWorldData = (value: unknown): WorldData => {
  ...
  const normalizeSeed = (seed: unknown): SceneEventSeed => {
    ...
    return {
      ...
      buildUpGoals: isStringArray(source.buildUpGoals) ? source.buildUpGoals : ['让玩家观察这个地点里的关系变化'],
      triggerHints: isStringArray(source.triggerHints) ? source.triggerHints : ['一次意外靠近让气氛变得暧昧'],
