...
export const buildTaskImagePrompt = ({
  ...
  return [
    ...
    `任务进度：${task.startMinutes} 到 ${task.endMinutes} 分钟刻度，当前在 ${task.currentMinutes}。总时长 ${task.durationMinutes} 分钟，过程拆分 ${task.segmentCount} 次。`,
    latestSegment ? `时间片：${latestSegment.fromLabel}-${latestSegment.toLabel}。` : '时间片：任务刚开始。',
    visualMoment,
    '画面小细节必须服务于任务内容本身；可以加入和当前任务直接相关的安全日常物件、环境和身体状态，不要把任务改写成学习、购物、聊天或无关剧情。',
